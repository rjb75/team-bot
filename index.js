...
var config = require("./config/config.js"); // imports config file

require("dotenv").config(); // to be removed when we switch over to git based keys

const fs = require("fs"); // require fs file system
...
bot.commands = new Discord.Collection(); // discord command collection
...
bot.on("guildMemberAdd", (member) => {
  var role = member.guild.roles.cache.find(
    (role) => role.name === "Looking For Team"
  ); //Adds Looking For Team upon joining
  var role2 = member.guild.roles.cache.find(
    (role) => role.name === "Unverified"
  ); //Adds Unverified Upon Joining
...
const doc = new GoogleSpreadsheet(
  "1hoTYv-70ggXe0ucVpecz0zr_Sq9ahxcFYC8dvAq3cYY"
);
...
async function verifyId(verification_code) {
  ...
  const targetRow = rows.filter(
    (row) => row.VerificationKey === verification_code
  )[0]; //Gets the row that exactly matches the verification code
...
async function verifyUser(verificationCode) {
  ...
  if (userData) {
    return [
      ...
      userData.SignedIn,
      userData.TeamNumber,
    ]; //Retrieves multiple value and stores them in array
  } else {
...
async function checkRole(TeamNumber, message, guild) {
  //
  let checkRole = await message.member.guild.roles.cache.find(
    (role) => role.name === `Team${TeamNumber}`
  ); //Attempts to finds the dynamic team role
  let Unverified = await message.member.guild.roles.cache.find(
    (role) => role.name === "Unverified"
  ); //Finds the Unverified Role
  let TeamStatus = await message.member.guild.roles.cache.find(
    (role) => role.name === "Looking For Team"
  ); //Finds the Looking For Team role
  if (checkRole) {
    ...
    return message.guild.members.cache
      ...
      .roles.add(checkRole); //If role exists assign them the role
    console;
  } else {
    ...
    checkRole = await message.member.guild.roles.cache.find(
      (role) => role.name === `Team${TeamNumber}`
    ); //Reassigns checkRole to the newly created roll
    message.guild.members.cache.get(message.author.id).roles.remove(TeamStatus); //Removes Looking For Team
    message.guild.members.cache.get(message.author.id).roles.remove(Unverified); //Removes unverified role
    return message.guild.members.cache
      .get(message.author.id)
      .roles.add(checkRole); //Now that the role exists, assign it to the person
  }
}
...
async function checkChannel(teamNumber, message, guild) {
  ...
  let category = await server.channels.cache.find(
      (c) => c.name == `Team-${teamNumber}` && c.type == "category"
    ), //See if there is any category already labeled "Team<DynamicNumber>"
    channel = await server.channels.cache.find(
      (c) => c.name == `workroom-${teamNumber}` && c.type == "text"
    ); ////See if there is any channel in that category labeled "Workroom-<DynamicNumber>"
  ...
  if (category && channel) {
    ...
    category = await server.channels.cache.find(
      (c) => c.name == `Team-${teamNumber}` && c.type == "category"
    ); //

    await message.guild.channels
      .create(`workroom-${teamNumber}`, {
        type: "text",
        permissionOverwrites: [
          {
            id: guild.id, // shortcut for @everyone role ID
...
bot.on("message", (message) => {
  ...
  if (!message.content.startsWith(config.prefix) || message.author.bot) {
    //This code checks to see if any text has the command prefix or if it is a bot triggering the command
  ...
  const args = message.content.slice(config.prefix.length).trim().split(" "); //Just parses spaces
  const command = args.shift().toLowerCase();

  if(!bot.commands.has(command)) {
    console.log('a invalid command was sent');
    return
  }; // if the commnad file does not exist return

  try {
    bot.commands.get(command).execute(message, args);
  } catch (error) {
    console.log(error);
    message.reply('There was an error trying to process your request!');
  }
  ...
  //       `You didn't provide any arguments, ${message.author}! Correct usage is !verify code \n \n===`
  //     );
  //   }
  //   //If there are too many arguments, we want to let the user know as well
  //   else if (args.length > 1) {
  //     return message.channel.send(
  //       `You provided too many arguments, ${message.author}! Correct usage is !verify code \n \n===`
  //     );
  //   }
  ...
  //           (role) => role.name === `${VerificationRoleName}`
  //         );
  ...
  //         if (role)
  //           message.guild.members.cache.get(message.author.id).roles.add(role); //Adds the verified role in
  //         message.author.send(
  //           `Thank you ${firstName}, you are now verified on the Hack Your Learning Discord! \n \n===`
  //         );
  ...
  //         //===================================================================================================================================================
  //         if (!teamNumber) {
  //           let Unverified = message.member.guild.roles.cache.find(
  //             (role) => role.name === "Unverified"
  //           );
  //           message.guild.members.cache
  //             .get(message.author.id)
  //             .roles.remove(Unverified); //Removes Unverified and Verifies them
  //           return message.channel.send(
  //             `${message.author} has been verified! Welcome to the Hack Your Learning Discord :smile: \n \n===`
  ...
  //         return message.channel.send(
  //           `Verification code has already been used ${message.author}. Please contact an administrator for help.\n\n===`
  //         );
  //       }
  //     } else {
  //       //This is the return error when a verification code is invalid
  //       return message.channel.send(
  //         `Verification Code Invalid ${message.author}, please try again or contact an administrator. \n \n===`
  //       ); //Rejection
  //     }
  //   });
  ...
  //     `Invalid Command on HackYourLearning Server, use !help for the list of commands.`
  //   ); //If no command is recognized, return error message
  // }
});
